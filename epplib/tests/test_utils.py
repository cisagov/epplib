...
class TestParseXMLMixin(TestCase):
    def test_find(self):
        ...
        self.assertEqual(
            cast(Element, ParseXMLMixin._find(element, "./epp:lang")).text, "en"
        )
    ...
    def test_find_all(self):
        ...
        self.assertEqual(
            [item.text for item in ParseXMLMixin._find_all(element, "./epp:lang")],
            ["en", "cs"],
        )
        self.assertEqual(
            [item.text for item in ParseXMLMixin._find_all(element, "./epp:version")],
            [None],
        )
        self.assertEqual(
            [item.text for item in ParseXMLMixin._find_all(element, "./epp:missing")],
            [],
        )
    ...
    def test_find_all_text(self):
        element = EM.svcMenu(EM.lang("en"), EM.lang("cs"), EM.version())
        self.assertEqual(
            ParseXMLMixin._find_all_text(element, "./epp:lang"), ["en", "cs"]
        )
    ...
    def test_find_attrib(self):
        ...
        self.assertEqual(
            ParseXMLMixin._find_attrib(element, "./epp:result", "code"), "1000"
        )
        self.assertEqual(
            ParseXMLMixin._find_attrib(element, "./epp:result", "other"), None
        )
        self.assertEqual(
            ParseXMLMixin._find_attrib(element, "./epp:other", "code"), None
        )
    ...
    def test_find_children(self):
        ...
        self.assertEqual(
            ParseXMLMixin._find_children(element, "./epp:purpose"), ["admin", "prov"]
        )
    ...
    def test_str_to_bool(self):
        ...
        with self.assertRaisesRegex(
            ValueError, 'Value "other" is not in the list of known boolean values\\.'
        ):
