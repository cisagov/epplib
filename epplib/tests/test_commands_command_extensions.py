    ...
    def test_valid(self):
        ...
        for subset in params:
            with self.subTest(params=subset):
                extension = CreateDomainEnumExtension(
                    **sub_dict(self.extension_params, subset)
                )
                self.assertRequestValid(
                    CreateDomain, self.command_params, extension=extension
                )
...
class TestUpdateContactMailingAddressExtension(XMLTestCase):
    ...
    def test_valid(self):
        ...
        extension_set = UpdateContactMailingAddressExtension(
            addr=ExtraAddr(**self.addr_params)
        )
    ...
    def test_data_set(self):
        extension = UpdateContactMailingAddressExtension(
            addr=ExtraAddr(**self.addr_params)
        )
        EM = ElementMaker(namespace=NAMESPACE.NIC_EXTRA_ADDR)
        expected = EM.update(
...
class TestRenewDomainEnumExtension(XMLTestCase):
    ...
    def test_valid(self):
        ...
        for subset in params:
            with self.subTest(params=subset):
                extension = RenewDomainEnumExtension(
                    **sub_dict(self.extension_params, subset)
                )
                self.assertRequestValid(
                    RenewDomain, self.command_params, extension=extension
                )
...
class TestUpdateDomainEnumExtension(XMLTestCase):
    ...
    def test_valid(self):
        ...
        for subset in params:
            with self.subTest(params=subset):
                extension = UpdateDomainEnumExtension(
                    **sub_dict(self.extension_params, subset)
                )
                self.assertRequestValid(
                    UpdateDomain, self.command_params, extension=extension
                )
