...
class DNSSECKeyData(PayloadModelMixin, ExtractModelMixin):
    ...
    def get_payload(self) -> Element:
        """Get Element representing the the model."""
        keyData = Element(
            QName(self.namespace, "keyData"), nsmap={"secDNS": self.namespace}
        )
...
class DSData(PayloadModelMixin, ExtractModelMixin):
    ...
    def get_payload(self) -> Element:
        """Get Element representing the model."""
        ds_data = Element(
            QName(NAMESPACE.SEC_DNS, "dsData"), nsmap={"secDNS": NAMESPACE.SEC_DNS}
        )
        ...
        SubElement(ds_data, QName(NAMESPACE.SEC_DNS, "digestType")).text = str(
            self.digestType
        )
    ...
    def extract(cls, element: Element) -> "DSData":
        ...
        return cls(
            ...
            keyData=cls._optional(
                DNSSECKeyData.extract, cls._find(element, "./secDNS:keyData")
            ),
        )
